...
import {
  ...
  useUploadIndustryLogo,
  useDeleteIndustryLogo,
} from "@/lib/api/queries/industryApi";
import {
  ESTADOS_BRASIL,
  CIDADES_POR_ESTADO,
} from "@/lib/utils/brazil-locations";
import { cn } from "@/lib/utils/cn";

const portfolioDisplaySettingsSchema = z.object({
  showName: z.boolean(),
...
const formSchema = z.object({
  ...
  socialLinks: z
    ...
    .max(5, "Máximo 5 redes sociais")
    .optional(),
  portfolioDisplaySettings: portfolioDisplaySettingsSchema.optional(),
});
...
const getSocialIcon = (url: string, name: string) => {
  ...
  if (lowerUrl.includes("tiktok") || lowerName.includes("tiktok"))
    return <span className="font-bold text-xs">Tk</span>; // Lucide might not have tiktok
...
export default function IndustryConfigPage() {
  ...
  const hasContact = Boolean(
    ...
    watchedWhatsapp?.trim(),
  );
  const hasLocation = Boolean(
    watch("addressCountry")?.trim() ||
    watch("addressState")?.trim() ||
    watch("addressCity")?.trim(),
  );

  // Detect if logo changed
  const isLogoDirty =
    selectedLogo !== null || previewLogoUrl !== originalLogoUrl;
  ...
  useEffect(() => {
    if (industry) {
      ...
      reset({
        ...
        portfolioDisplaySettings: industry.portfolioDisplaySettings || {
          showName: true,
          showDescription: false,
          showLogo: false,
          showCNPJ: false,
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (err?.response?.status === 413) {
        showError(
          "A imagem ainda é muito grande. Tente reduzir o zoom da edição.",
        );
      } else {
        showError(t("error"));
      }
  ...
  const stateOptions = ESTADOS_BRASIL.map((e) => ({
    label: e.nome,
    value: e.sigla,
  }));
  const cityOptions =
    watchedState && CIDADES_POR_ESTADO[watchedState]
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className={cn(
                          "flex-1 px-3 py-2.5 bg-slate-50 border focus:border-[#C2410C] focus:bg-white outline-none text-sm transition-colors lowercase",
                          errors.slug ? "border-rose-500" : "border-slate-200",
                        )}
                        onChange={(e) => {
                          // Normalizar: lowercase, substituir espaços por hífens, remover caracteres especiais
                  ...
                  <div>
                    ...
                    {showCNPJInfo && (
                      <div className="mb-3 p-3 bg-slate-100 border border-slate-200 text-xs text-slate-700">
                        Para empresas brasileiras, use o modo BR com máscara de
                        CNPJ. Para empresas estrangeiras, alterne para modo
                        Internacional e insira o número de identificação
                        empresarial do seu país.
                      </div>
                    )}
            ...
            <div className="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Conecte suas redes sociais (máx. 5)
                  </p>
                </div>
              </div>
              <button
                ...
                <div className="space-y-3">
                  {fields.map((field, index) => {
                    ...
                    return (
                      <div
                        key={field.id}
                        className="group relative grid grid-cols-1 md:grid-cols-[200px_1fr_auto] gap-3 items-start p-3 rounded-lg border border-slate-100 bg-slate-50 hover:border-slate-200 transition-all"
                      >
                        <div>
                          <input
                            ...
                            className={cn(
                              ...
                              errors.socialLinks?.[index]?.name
                                ? "border-rose-500"
                                : "border-slate-200 focus:border-[#C2410C]",
                            )}
                          />
                          {errors.socialLinks?.[index]?.name && (
          ...
          <div className="bg-white border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-semibold text-[#121212]">
                    {t("portfolioVisibility.title")}
                  </h2>
                  <p className="text-xs text-slate-500">
                    {t("portfolioVisibility.subtitle")}
                  </p>
                </div>
              </div>
            </div>
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                <div>
                  <p className="font-medium text-sm text-slate-800">
                    {t("portfolioVisibility.isPublished")}
                  </p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {t("portfolioVisibility.isPublishedDesc")}
                  </p>
                </div>
                <Controller
                  name="portfolioDisplaySettings.isPublished"
                  control={control}
                  render={({ field }) => (
                    <button
                      type="button"
              ...
              <div className="space-y-4">
                <p className="text-sm font-medium text-slate-700">
                  {t("portfolioVisibility.displayOptions")}
                </p>
                ...
                <div className="flex items-center justify-between py-2 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <span className="text-sm text-slate-600">
                      {t("portfolioVisibility.showLogo")}
                    </span>
                    {!previewLogoUrl && (
                      <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded">
                        {t("portfolioVisibility.noLogoWarning")}
                      </span>
                    )}
                  </div>
                  <Controller
                    name="portfolioDisplaySettings.showLogo"
                    control={control}
                    render={({ field }) => (
                      <button
                        type="button"
                        ...
                        className={cn(
                          "relative inline-flex h-5 w-9 items-center rounded-full transition-colors",
                          !previewLogoUrl && "opacity-50 cursor-not-allowed",
                ...
                {watch("portfolioDisplaySettings.showLocation") &&
                  hasLocation && (
                    <div className="ml-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
                      ...
                      <Controller
                        name="portfolioDisplaySettings.locationLevel"
                        control={control}
                        render={({ field }) => (
                          <select
                            {...field}
                            className="w-full px-3 py-2 bg-white border border-slate-200 focus:border-[#C2410C] outline-none text-sm transition-colors rounded-md"
                          >
                            <option value="country">
                              {t("portfolioVisibility.levelCountry")}
                            </option>
                            <option value="state">
                              {t("portfolioVisibility.levelState")}
                            </option>
                            <option value="city">
                              {t("portfolioVisibility.levelCity")}
                            </option>
                            <option value="full">
                              {t("portfolioVisibility.levelFull")}
                            </option>
                          </select>
                        )}
                      />
                      <p className="text-xs text-slate-500 mt-2">
                        {t("portfolioVisibility.locationLevelDesc")}
                      </p>
                    </div>
                  )}
              </div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      >
                        {typeof window !== "undefined"
                          ? `${window.location.origin}/${watchedSlug}/public-portfolio`
                          : `/${watchedSlug}/public-portfolio`}
                      </code>
                      {savedPublished ? (
                        <a
                          href={`/${industry?.slug || watchedSlug}/public-portfolio`}
                          target="_blank"
              ...
              <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-100 rounded-lg">
                ...
                <div className="text-xs text-blue-700">
                  <p className="font-medium mb-1">
                    {t("portfolioVisibility.infoTitle")}
                  </p>
                  <p>{t("portfolioVisibility.infoDesc")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 sticky bottom-6 bg-white/95 backdrop-blur-md p-4 rounded-xl shadow-xl border border-slate-200/50 mx-6 mb-6">
            ...
            <button
              ...
              className={cn(
                ...
                !hasChanges || isUploading || updateConfig.isPending
