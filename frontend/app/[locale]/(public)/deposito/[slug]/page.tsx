...
import { formatArea, formatDimensions, calculateTotalArea } from '@/lib/utils/formatDimensions';
...
export default function DepositCatalogPage() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [depositData, batchesData] = await Promise.all([
        apiClient.get<PublicDeposit>(`/public/deposits/${slug}`),
        apiClient.get<PublicBatch[]>(`/public/deposits/${slug}/batches`),
      ]);
                    ...
                    <div className="lg:col-span-5 flex flex-col space-y-8 md:space-y-14 lg:space-y-16 lg:sticky lg:top-40 order-1">
                      ...
                      <div className="space-y-12 border-t border-slate-200 pt-4 md:pt-12 animate-in fade-in duration-700 delay-200">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-12 relative">
                          ...
                          <div>
                            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-3">Chapas Disponíveis</p>
                            <p className="text-2xl font-serif text-emerald-600">{selectedBatch.availableSlabs}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="lg:col-span-7 space-y-8 order-2 min-w-0">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="aspect-[3/2] sm:aspect-square md:aspect-[2/1] bg-slate-100 flex items-center justify-center text-slate-400 font-serif italic text-lg border border-slate-200/50">
                          ...
                          <span>Área: {formatArea(calculateTotalArea(selectedBatch.height, selectedBatch.width))}</span>
                        </div>
                        ...
                        <div className="mx-auto">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-8">
                            ...
                            <div className="text-center space-y-3 border-b md:border-b-0 md:border-r border-slate-200 pb-8 md:pb-0">
                              ...
                              <p className="text-2xl md:text-3xl font-serif text-emerald-600 tracking-tight">
                                {selectedBatch.availableSlabs}
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                  ...
                                  <p className="text-sm font-medium text-[#121212]">{deposit.name}</p>
                                </div>
                              </div>
      ...
      <nav className="fixed top-0 w-full bg-white/90 backdrop-blur-md z-50 border-b border-slate-100 animate-in slide-in-from-top duration-500">
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-20 md:h-24 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center">
            ...
            {deposit.logoUrl && !isPlaceholderUrl(deposit.logoUrl) && (
              <img
                src={deposit.logoUrl}
                alt={deposit.name}
        ...
        <section className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 pt-4 pb-2">
          <div className="animate-in slide-in-from-left duration-700">
            <div className="flex items-center space-x-4 mb-3">
              <span className="h-px w-10 bg-[#121212]"></span>
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#121212]">Catálogo Oficial</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-4">
              <div>
                ...
                {(deposit.city || deposit.state) && (
                  <div className="flex items-center gap-2 text-slate-500">
                    <MapPin className="w-4 h-4" />
                    <span className="text-base font-light">{[deposit.city, deposit.state].filter(Boolean).join(', ')}</span>
                  </div>
                )}
              </div>
              ...
              <div className="text-right">
                ...
                <p className="text-3xl md:text-4xl font-serif text-[#121212]">{batches.length}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 py-6">
          {batches.length === 0 ? (
            <div className="bg-white border border-slate-200 p-16 text-center">
              ...
              <p className="text-slate-500">Este catálogo ainda não possui lotes públicos</p>
            </div>
          ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="aspect-[3/4] bg-slate-100 relative overflow-hidden mb-6">
                        ...
                        {/* Available Slabs Badge */}
                        <div className="absolute top-4 left-4">
                          <div className="bg-emerald-500/90 backdrop-blur-md px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white">
                      ...
                      <div>
                        ...
                        <div className="flex items-center text-xs font-bold uppercase tracking-widest text-slate-400">
                          ...
                          {formatDimensions(batch.height, batch.width, batch.thickness)}
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className="flex items-center gap-4">
            ...
            {deposit.logoUrl && !isPlaceholderUrl(deposit.logoUrl) && (
              <>
                ...
                <img src={deposit.logoUrl} alt={deposit.name} className="h-6 object-contain opacity-60" />
              </>
            )}
          </div>
